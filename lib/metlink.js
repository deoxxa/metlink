...
Metlink.prototype.get_transport_line_list = Metlink.prototype.getTransportLineList = function(type, done) {
  request(this.base_url + "/getTransportLineList.php?transportType=" + encodeURIComponent(type), function(err, xhr, data) {
    if (err) {
      return done(err);
...
Metlink.prototype.get_stop_list_from_transport_line = Metlink.prototype.getStopListFromTransportLine = function(type, line, done) {
  request(this.base_url + "/getStopListFromTransportLine.php?" + ["transportType=" + encodeURIComponent(type), "transportLine=" + encodeURIComponent(line)].join("&"), function(err, xhr, data) {
    if (err) {
      return done(err);
    } else if (xhr.statusCode !== 200) {
      return done(Error("request failed"));
